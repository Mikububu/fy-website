/**
 * Automated Substack Migration Script (Using curl)
...
// Utility: Fetch URL using curl
function fetchUrl(url) {
  try {
    const result = execSync(`curl -sL "${url}"`, { encoding: 'utf8', maxBuffer: 10 * 1024 * 1024 });
    return result;
  } catch (error) {
...
async function fetchRssFeed() {
  ...
  try {
    const xml = fetchUrl(CONFIG.rssFeedUrl);
...
async function processMedia(post) {
  ...
  // Note: Image downloading skipped for now (would use curl to download)
  console.log(`   ℹ️  Image downloading not yet implemented`);
...
// Main execution
async function main() {
  console.log('🚀 Starting Automated Substack Migration (curl version)\n');
